...
class TestGerritWorkflowIntegration:
    ...
    async def test_validate_tag_with_gerrit_failure(self):
        """Test complete tag validation with failed Gerrit verification."""
        config = ValidationConfig(
            require_gerrit=True,
            gerrit_server="gerrit.onap.org",
            require_signed=True,
        ...
        # Mock failed Gerrit verification
        mock_gerrit_result = KeyVerificationResult(
    ...
    async def test_validate_tag_combined_github_gerrit(self):
        ...
        assert result.is_valid is True
        # Should have GitHub verification as primary
        assert result.key_verification.service == "github"
        # But should have info about Gerrit verification in messages
        assert any("verified on Gerrit server" in info for info in result.info)

    @pytest.mark.asyncio
    async def test_validate_tag_gerrit_server_discovery(self):
        """Test tag validation with Gerrit server auto-discovery."""
        config = ValidationConfig(
            require_gerrit=True,
            # No gerrit_server specified - should auto-discover
        )
        workflow = ValidationWorkflow(config)
